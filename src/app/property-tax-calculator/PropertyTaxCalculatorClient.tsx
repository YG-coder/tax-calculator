...
// src/app/property-tax-calculator/PropertyTaxCalculatorClient.tsx
// 계산 로직은 calc.ts에 격리(검증 완료). 레이아웃·컬러는 기존 계산기 컨벤션(slate)에 맞춤.
  ...
  return (
    <main className="max-w-3xl mx-auto px-4 py-10">
      <div className="mb-8">
        ...
        <h1 className="text-3xl font-bold text-slate-900 mb-3 leading-tight">재산세 계산기</h1>
      ...
      <div className="space-y-6">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-6 space-y-5">
          ...
          <label className="block">
            <span className="text-sm font-medium text-slate-700">주택 공시가격</span>
          ...
          <label className="block">
            <span className="text-sm font-medium text-slate-700">소유지분율</span>
            <div className="mt-1 flex items-center gap-2">
              <input
                inputMode="numeric"
                value={ownershipPct || ''}
          ...
          <div className="rounded-xl bg-slate-50 p-4">
            <label className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-slate-700">세부담상한 적용 (전년도 세액 입력)</span>
            </label>
        ...
        {result && (
          <section className="rounded-2xl border border-slate-200 bg-white p-6 space-y-4">
            <h2 className="text-lg font-bold text-slate-800">계산 결과</h2>
            ...
            <div className="rounded-xl bg-slate-50 p-4 text-sm text-slate-600 space-y-1">
              <p className="font-semibold text-slate-700">계산 상세</p>
...
function Row({ label, value, strong }: { label: string; value: string; strong?: boolean }) {
  return (
    <div className="flex items-center justify-between py-2.5">
      <dt className={`text-sm ${strong ? 'font-semibold text-slate-900' : 'text-slate-600'}`}>
        {label}
      </dt>
      <dd className={`text-sm tabular-nums ${strong ? 'font-semibold text-slate-900' : 'text-slate-800'}`}>
        {value}
      </dd>
    </div>
  )
